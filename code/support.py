...
def get_angle_deg(anchor, point):
    # second point - anchor
    angle = math.atan2(-point[1] - -anchor[1], point[0] - anchor[0])  # flip y axis as a result of pygame flipped axis
    angle *= 180 / math.pi  # converts to degrees
    return angle + 270  # rotate to conform to pygame orientation of unit circle ???


def get_angle_rad(anchor, point):
    return math.radians(get_angle_deg(anchor, point))
